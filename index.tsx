  ...
  const generateAndStartQuiz = async (settings: QuizSettings) => {
    ...
    try {
        ...
        const errorMessage = e.message?.includes('API key not valid') 
            ? "The configured API key is invalid. Please contact the developer."
...
const HomeScreen: React.FC<{
  images: ImageFile[], onAddImageClick: () => void, onCameraClick: () => void, onRemoveImage: (id: string) => void, onGenerateClick: () => void, error: string | null, isLoading: boolean
}> = ({ images, onAddImageClick, onCameraClick, onRemoveImage, onGenerateClick, error, isLoading }) => (
...
const HolographicBookModel: React.FC<{ progress: number; }> = ({ progress }) => {
    ...
    useEffect(() => {
        ...
        const modelUrl = 'https://vazxmixjsiawhamofees.supabase.co/storage/v1/object/public/models/book/model.gltf';
